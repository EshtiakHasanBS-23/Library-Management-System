...
import { CheckCircle2, RefreshCcw, BookOpen, Search } from "lucide-react";
import Sidebar from "../../components/DashboardSidebar/DashboardSidebar";
import Pagination from "../../components/Pagination/Pagination";
...
export default function ManageFeature() {
  ...
  const toggleFeatured = async (book) => {
    const token = localStorage.getItem("token");
    const newStatus = !book.is_featured;
    try {
      await axios.patch(
      `http://127.0.0.1:8000/books/${book.id}/feature?featured=${newStatus}`,
      {}, // body can be empty
      { headers: { Authorization: `Bearer ${token}` } }
    );
      setBooks((prev) =>
        prev.map((b) =>
          b.id === book.id ? { ...b, is_featured: newStatus } : b
        )
      );
      showToast(
        newStatus ? "Book marked as Featured" : "Book removed from Featured"
      );
    } catch (err) {
  ...
  const filteredBooks = useMemo(() => {
    ...
    return books.filter((b) => {
      ...
      if (filter === "featured") return matchesSearch && b.is_featured;
      if (filter === "unfeatured") return matchesSearch && !b.is_featured;
  ...
  const startIdx = (page - 1) * PAGE_SIZE;
  const endIdx = Math.min(startIdx + PAGE_SIZE, totalItems);
  const pageItems = filteredBooks.slice(startIdx, endIdx);
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full border border-gray-300 rounded px-10 py-2 focus:outline-none focus:ring-2 focus:ring-sky-400"
            />
          </div>
          <select
            value={filter}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {loading &&
            Array.from({ length: 8 }).map((_, i) => (
              <div
                key={`sk-${i}`}
                className="rounded-lg border p-4 bg-gray-200 animate-pulse"
              />
            ))}
          ...
          {!loading &&
            pageItems.map((book) => (
              <div
                key={book.id}
                className={`border rounded-lg p-4 cursor-pointer flex flex-col items-center transition ${
                  book.is_featured
              ...
              >
                <img
                  src={book.image ? `http://localhost:8000${book.image}` : PLACEHOLDER_IMG}
                  alt={book.title}
                ...
                <button
                  ...
                  className={`mt-2 px-3 py-1 rounded-md text-white text-sm ${
                    book.is_featured ? "bg-red-600 hover:bg-red-500" : "bg-green-600 hover:bg-green-500"
                  }`}
                >
